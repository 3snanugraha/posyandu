-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- Host: localhost
-- Generation Time: Jan 25, 2024 at 09:29 PM
-- Server version: 10.4.28-MariaDB
-- PHP Version: 8.0.28

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `posyandu_app`
--

-- --------------------------------------------------------

--
-- Table structure for table `tblPeriksaLansia`
--

CREATE TABLE `tblPeriksaLansia` (
  `id_pemeriksaan` int(11) NOT NULL,
  `lansia_NIK` varchar(16) NOT NULL,
  `periksa_tb` varchar(5) NOT NULL,
  `periksa_bb` varchar(5) NOT NULL,
  `body_fat` varchar(5) NOT NULL,
  `tensi` varchar(5) NOT NULL,
  `gula_darah` varchar(5) NOT NULL,
  `status_periksa` varchar(20) NOT NULL,
  `tanggal_periksa` date NOT NULL,
  `keterangan` text NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Indexes for dumped tables
--

--
-- Indexes for table `tblPeriksaLansia`
--
ALTER TABLE `tblPeriksaLansia`
  ADD PRIMARY KEY (`id_pemeriksaan`),
  ADD KEY `fk_lansia` (`lansia_NIK`);

--
-- AUTO_INCREMENT for dumped tables
--

--
-- AUTO_INCREMENT for table `tblPeriksaLansia`
--
ALTER TABLE `tblPeriksaLansia`
  MODIFY `id_pemeriksaan` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
